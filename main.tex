...
\begin{adjustwidth}{-2cm}{-2cm}
    \begin{minipage}{\linewidth}
        ...
        \captionsetup{width=0.9\linewidth, justification=centering} 
        \captionof{figure}{Network level descriptive statistics for the Harris and Trump hashtag co-occurrence social networks. Average Clustering Coefficient is approximated with networkx, and the number shown here is an approximate lower bound for the true value. Note that edges are weighted by the number of co-occurrences for any two hashtags. One TikTok repost equates to one co-occurrence.}
        \label{network_stats}
    \end{minipage}
\end{adjustwidth}
\end{figure}
...
\begin{adjustwidth}{-2cm}{-2cm}
    \begin{minipage}{\linewidth}
        ...
        \captionsetup{width=0.9\linewidth, justification=centering} 
        \captionof{figure}{* $p < 0.5$, ** $p < 0.01$, *** $p < 0.001$}
        \label{network_t_test}
    \end{minipage}
\end{adjustwidth}
\end{figure}

Figure \ref{network_t_test} demonstrates that the Trump co-occurrence network, alongside being denser, has a higher average degree and higher average betweenness than the Harris co-occurrence network. Looking at Figure \ref{degree_density},  we can see that this difference in degree holds across the entire distribution and not just at tail ends. 
